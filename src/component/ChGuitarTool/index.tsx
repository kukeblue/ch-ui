...
export default ({}: {}) => {
  ...
  const buildQuestion = () => {
    form.validateFields().then(values => {
      ...
      let rowIndex = Math.floor(Math.random() * 24);

      if (values.fixedLine) {
        lineIndex = Number(values.fixedLine) - 1;
      }
      if (values.fixedRow) {
        rowIndex = Math.floor(Math.random() * values.fixedRow);
      }
      let gridIndex = Math.floor(Math.random() * 24);
      setQuestion(`请找到第${lineIndex + 1}弦，第${rowIndex + 1}品的音`);
    });
  };
  useEffect(() => {
    buildQuestion();
